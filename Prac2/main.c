...

{
  ...
  while (1)
  {
	...
	if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET) {
	  // Toggle the half_second_flag
	  flag = !flag;
	  // Wait for the button to be released
	  while (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET);
	}

	// If the half_second_flag is set, change the ARR value of TIM16 to 500
	if (flag) {
	  TIM16->ARR = 500;
	}
	// Otherwise, change the ARR value of TIM16 to 1000
	else {
	  TIM16->ARR = 1000;
	}
  /* USER CODE END 3 */
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t out = read_from_address(index);

	if(out == patterns[index])
	{
		LL_GPIO_WriteOutputPort(GPIOB, out);
	}
	else
	{
		LL_GPIO_WriteOutputPort(GPIOB, 0b00000001);
	}
